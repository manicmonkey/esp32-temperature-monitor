...
#define TIME_TO_SLEEP  60.0        /* Time ESP32 will go to sleep (in seconds) */
...
void send_to_mqtt(TemperatureReading reading) {
    EventBits_t uxBits;
...
void setup() {
    ...
    esp_err_t err = esp_sleep_enable_timer_wakeup(TIME_TO_SLEEP * uS_TO_S_FACTOR);
    ESP_LOGI(TAG, "Sleep config: %s", esp_err_to_name(err));
...
void loop() {
    ...
    start_wifi();
    localTemperature->start();
    remoteTemperature->start();

    displayTemperature();
    submitTemperature(localTemperature, "local");
    submitTemperature(remoteTemperature, "remote");

    localTemperature->stop();
    remoteTemperature->stop();
    stop_wifi();
